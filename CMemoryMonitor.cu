 ...
 */
unsigned int CMemoryMonitor::get(void* dest,
 ...
 */
unsigned int CMemoryMonitor::set(unsigned int x,
 ...
 */
void CMemoryMonitor::copyReferences(MemoryMonitor* m)
{
   MonitorBucket* v;

   v = m->getReferences();

   cudaMemcpy(&references, &v, sizeof(MonitorBucket)*m->getNumBuckets(),
